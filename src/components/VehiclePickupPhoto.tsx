...
import { Camera, X, Loader2, Check, ImageIcon, Play } from 'lucide-react';
...
interface VehiclePickupPhotoProps {
  bookingId: string;
  vehicleName: string;
  isOpen: boolean;
  onClose: () => void;
...
const VehiclePickupPhoto = ({
  bookingId,
  vehicleName,
  isOpen,
  onClose,
  onComplete,
}: VehiclePickupPhotoProps) => {
  const { user } = useAuth();
  ...
  const uploadPhotos = async () => {
    ...
    try {
      ...
      for (let i = 0; i < photos.length; i++) {
        ...
        const fileName = `${user.id}/${bookingId}/pickup-${Date.now()}-${i}.${fileExt}`;
      ...
      // Save photo records to database with pickup type
      ...
      toast.success('Fahrzeug erfolgreich übernommen! Gute Fahrt!');
      onComplete();
      handleClose();
    } catch (err) {
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="glass max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Fotografiere das Fahrzeug <strong>{vehicleName}</strong> vor der Übernahme, um den aktuellen Zustand zu dokumentieren.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {/* Notes */}
          <div>
            <Label htmlFor="pickup-notes">Anmerkungen zu bestehenden Schäden (optional)</Label>
            <Textarea
              id="pickup-notes"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="z.B. Kleine Delle an der Fahrertür bereits vorhanden..."
              rows={3}
            />
          </div>
          ...
          <div className="p-3 rounded-lg bg-primary/10 border border-primary/20 text-sm">
            <p className="text-foreground">
              <strong>Wichtig:</strong> Dokumentiere vorhandene Schäden vor der Fahrt. Diese Fotos schützen dich bei der Rückgabe vor unberechtigten Schadensansprüchen.
            </p>
          </div>
        </div>
...
export default VehiclePickupPhoto;
